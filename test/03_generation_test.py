...
def test_generation():
    test_util.generate_for("./test/_protocols/example.toml", "WireMessage")
...
def test_harness_compilation():
    ...
    for label, writer in beschi.writers.all_writers.items():
        ...
        invoke_build = [
            ...
            "--srcfile", f"harness{writer.default_extension}",
            "--libfile", f"WireMessage{writer.default_extension}",
        ]
