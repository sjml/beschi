...
* You can call the static `fromBytes` on `Message` itself instead of one of the specific message types; the only caveat is that, if you do so, it expects a buffer that starts with the tag of the message type.
