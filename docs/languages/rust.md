...
* Messages are defined as an enum with data, so instead of having to examine and cast them, you use Rust's usual pattern-matching to figure out what kind of message structure you're looking at. 
    ...
    * Because of the way the enum is set up, you can call the static `from_bytes` on `Message` itself instead of one of the specific message types; the only caveat is that, if you do so, it expects a buffer that starts with the tag of the message type.
